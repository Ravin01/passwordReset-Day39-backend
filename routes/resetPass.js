...
import { registerModel } from "../db/modles.js";
...
resetPasswordRoute.post('/', async(req,res)=>{
    ...
    try{
        const data = await registerModel.find({email : userEmail[0]}, { _id : 0, email : 1, userName : 1, registerId : 1})
    console.log(data)
    // const updatePassword = {...data, password : payload}
    // console.log(updatePassword)
    bcrypt.hash(payload, 10, async(err,hash)=>{
        if(err){
            res.status(500).send('error while updating password')
        }
        await registerModel.findOneAndUpdate({email : userEmail[0]}, {...data, password : hash})
